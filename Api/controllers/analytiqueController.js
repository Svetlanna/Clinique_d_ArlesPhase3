...
export const validerDiagnosticNuit = async (req, res) => {
    ...
    try {
        ...
        // !!!!
        // Svetlana : Générer rapport PDF dans un dossier à part dans Api/data/rapport-pdf/
        /// !!!
        ...
        res.status(200).json({
            status: 'success',
            data: { id_nuit, pdf_path: pdfPath, indicateurs: { ...indicateurs, severite_iah }, galaxie },
        });
    } catch (error) {
        res.status(500).json({ status: 'error', message: error.message });
    }
