    ...
    try:
        ...
        if id_nuit:
            ...
            query = """
                ...
                LIMIT 1
            """
            df = pd.read_sql(query, myconn, params=[id_patient])
...
def entrainer_tous_modeles(df, top_comorbidites):
    ...
    for comorb in top_comorbidites:
        ...
        if model:
            models[comorb] = model
            joblib.dump(model, MODEL_PATH / f"modele_{comorb}.pkl")
    return models
...
def afficher_prediction_comorbidites(id_patient=None):
    ...
    try:
        if id_patient is None:
            df_patients = pd.read_sql("SELECT id_patient, nom, prenom FROM patient", myconn)
